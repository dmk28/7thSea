...
class Weapon(DefaultObject):
    ...
    def at_object_creation(self):
        ...
        self.db.description = "A generic weapon"
        self.db.weapon_type = "Generic"
        self.db.traits = {}
        self.db.flameblade_active = False
    ...
    def calculate_damage(self, wielder):
        """
        Calculate the damage for this weapon based on the wielder's attributes and skills.
        """
        if self.meet_requirements(wielder):
            return wielder.db.traits['brawn'] + self.db.damage_bonus
        else:
            return self.db.damage  # No damage bonus if requirements aren't met

    def meet_requirements(self, wielder):
        """
        Check if the wielder meets the weapon's requirements.
        """
        for skill, required_level in self.db.requirements.get("skills", {}).items():
            if wielder.db.skills.get(skill, 0) < required_level:
                return False
        return True

    def get_attack_trait(self, wielder):
        """
...
class HeavySword(Weapon):
    def at_object_creation(self):
        super().at_object_creation()
        self.db.weapon_type = "Heavy Weapons"
        self.db.damage_keep = 2
        self.db.damage = 3
        self.db.attack_skill = "Attack (Heavy Weapons)"
        self.db.parry_skill = "Parry (Heavy Weapons)"
        self.db.cost = 1500
        self.db.damage_bonus = 0
class Sword(Weapon):
    def at_object_creation(self):
        """Set up the default attributes for a sword."""
        super().at_object_creation()
...
class Dagger(Weapon):
    def at_object_creation(self):
        '''Attributes for a dagger'''
        super().at_object_creation()
        self.db.weapon_type = "Knife"
        self.db.damage = 1
        self.db.damage_keep = 1
        self.db.attack_skill = "Attack (Knife)"
        self.db.parry_skill = "Parry (Knife)"
class Firearm(Weapon):
    def at_object_creation(self):
        super().at_object_creation()
        self.db.weapon_type = "Firearm"
        self.db.damage = 2
        self.db.damage_keep = 2
        self.db.attack_skill = "Attack (Firearms)"
        self.db.parry_skill = None
